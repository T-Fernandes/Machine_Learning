...
packages <- c("caret", "RSNNS", "class", "ks", "naivebayes", "MASS")
...
## Classifier: KNN
k = 3
Y_estimado_Knn = class::knn(X_treino_num_norm, X_teste_num_norm, Y_treino, k = k, prob = T)

## Confusion Matrix
Y_teste2 = as.factor(Y_teste)
(a1 = caret::confusionMatrix(Y_estimado_Knn, Y_teste2))
(e1 = ks::compare(Y_teste, Y_estimado_Knn))
